"""
Exact Coxeter enumerator for free {4,5} hyperbolic polyplets using 
Redelmeier Fixed Enumeration and Burnside's Lemma (Orbit-Stabilizer).

This script replaces Canonicalization with a Graph Spanning Tree (Redelmeier) 
and mathematically resolves the free count analytically, saving massive amounts 
of memory for deep combinatorial searches.
"""
...
class ExactBurnsideEnumerator:
    ...
    def anchor_transforms(self, target: int) -> tuple[Element, ...]:
        """Returns the 8 transformations that map the target cell to the base cell."""
    ...
    def shape_stabilizer_size(self, shape: frozenset[int]) -> int:
        ...
        Returns the exact integer size of the stabilizer group |Sym(P)|.
        """
        sym_count = 0
        for target in shape:
